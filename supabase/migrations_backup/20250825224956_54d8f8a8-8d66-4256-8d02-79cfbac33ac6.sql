-- Fix the create_default_categories_for_user function to handle potential duplicates
...
BEGIN
    ...
    -- Create Income group
    INSERT INTO category_groups (user_id, name, description, category_type, icon, sort_order)
    VALUES (target_user_id, 'Income', 'All income sources', 'income', '💰', 1)
    ...
    INSERT INTO category_groups (user_id, name, description, category_type, icon, sort_order)
    VALUES (target_user_id, 'Expenses', 'All expense categories', 'expense', '💸', 2)
    ON CONFLICT DO NOTHING
    ...
    INSERT INTO category_groups (user_id, name, description, category_type, icon, sort_order)
    VALUES (target_user_id, 'Transfers', 'Account transfers', 'transfer', '🔄', 3)
    ON CONFLICT DO NOTHING
