-- Add Investment category group
INSERT INTO public.category_groups (key, name, sort_order) 
VALUES ('investments', 'Investments', 60) 
ON CONFLICT (key) DO NOTHING;

-- Update seed_default_categories function to include Investment categories
...
BEGIN
  ...
  -- Create Investment buckets and categories
  INSERT INTO public.category_buckets (user_id, group_id, name, sort_order)
  VALUES (v_user, v_investments_group, 'Stocks & ETFs', 10)
  RETURNING id INTO b_stocks_etfs;

  INSERT INTO public.category_buckets (user_id, group_id, name, sort_order)
  VALUES (v_user, v_investments_group, 'Retirement Accounts', 20)
  RETURNING id INTO b_retirement;
  ...
  VALUES 
    -- Stocks & ETFs
    (v_user, b_stocks_etfs, 'Individual Stocks', 10),
    (v_user, b_stocks_etfs, 'ETFs', 20),
    (v_user, b_stocks_etfs, 'Mutual Funds', 30),
    (v_user, b_stocks_etfs, 'Index Funds', 40),
    
    -- Retirement Accounts
    (v_user, b_retirement, '401(k)', 10),
    (v_user, b_retirement, 'IRA', 20),
    (v_user, b_retirement, 'Roth IRA', 30),
    (v_user, b_retirement, 'Pension', 40),
    ...
    (v_user, b_alternative, 'Real Estate Investment', 10),
    (v_user, b_alternative, 'Commodities', 20),
    (v_user, b_alternative, 'Private Equity', 30),
    (v_user, b_alternative, 'Hedge Funds', 40);
